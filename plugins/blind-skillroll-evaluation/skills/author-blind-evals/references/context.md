...
Run offline validation, then record each `.eval.md` path and content hash in a case
manifest. Unchanged starter text, missing sections, invalid metadata, or an
authoring assumption that is absent from Input or World blocks the live run.
Artifact rendering, real repository commands, and real service changes need
their own evidence and should not be claimed by a semantic case.
